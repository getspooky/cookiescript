...
function getDestination() {
  return path.resolve(__dirname, '../../', 'server', 'models');
}
 ...
 */
function getStub() {
  return path.resolve(__dirname, '../stubs', 'model.stub');
}
    ...
    ReadFile(getStub(), 'utf8')
      .then(data => {
        ...
        const result = data
          ...
          .replace(/SchemaFileName/g, `${moment().format('YYYY[_]MM[_]DD')}_create_${file}_schema`)
          .replace(/MODEL_NAME/g, str_capitalize(file));
        ...
        WriteFile(destination, result, 'utf8').then(() =>
          log(chalk.green('Model successfully generated!', destination))
        );
      })
      .catch(err => error(err));
  },
